...
if __name__ == '__main__':
    ...
    for category in categories:
        ...
        plt.scatter(train_embedding[:, 0], train_embedding[:, 1], c='b', label='Train', s=2, alpha=0.1)
        plt.scatter(test_good_embedding[:, 0], test_good_embedding[:, 1], c='g', label='Test-Good', s=2, alpha=0.1)
        plt.scatter(test_bad_embedding[:, 0], test_bad_embedding[:, 1], c='k', label='Test-Bad', s=2, alpha=0.1)
        plt.scatter(few_shot_embedding_2[:, 0], few_shot_embedding_2[:, 1], c='r', label='Few-Shot', s=4, alpha=1)
        plt.legend(fontsize='medium')
        plt.xticks([])
        plt.yticks([])
        plt.title('{} (2 Shot)'.format(category), fontsize=12)
        plt.savefig('./work_dir/vis_result/{}_{}_not_augment.png'.format(category, 2), bbox_inches='tight', pad_inches=0.1)
        plt.clf()
    
    
        fig = plt.figure(figsize=(5, 4), dpi=400, facecolor='w', edgecolor='k')
        plt.scatter(train_embedding[:, 0], train_embedding[:, 1], c='b', label='Train (All)', s=2, alpha=0.1)
        ...
        plt.scatter(few_shot_embedding_4[:, 0], few_shot_embedding_4[:, 1], c='r', label='FewShot', s=4, alpha=1)
        plt.legend(fontsize='medium')
        plt.xticks([])
        plt.yticks([])
        plt.title('{} (8 Shot)'.format(category), fontsize=12)
        ...
        # plt.scatter(test_good_embedding[:, 0], test_good_embedding[:, 1], c='g', label='Test-Good', s=2, alpha=1)
        # plt.scatter(test_bad_embedding[:, 0], test_bad_embedding[:, 1], c='r', label='Test-Bad', s=2, alpha=1)
        plt.scatter(few_shot_embedding_8[:, 0], few_shot_embedding_8[:, 1], c='r', label='FewShot', s=4, alpha=1)
        plt.legend(fontsize='medium')
        plt.xticks([])
        plt.yticks([])
        plt.title('{} (16 Shot)'.format(category), fontsize=12)
