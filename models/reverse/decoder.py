...
__all__ = ['DecBasicBlock', 'DecBottleneck', 'DecResNet']
...
class DecBottleneck(nn.Module):
    ...
    def __init__(
        ...
        super(DecBottleneck, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
...
class DecResNet(nn.Module):
    def __init__(
        self,
        block: Type[Union[DecBasicBlock, DecBottleneck]],
        layers: List[int],
        ...
        if zero_init_residual:
            for m in self.modules():
                if isinstance(m, DecBottleneck):
                    nn.init.constant_(m.bn3.weight, 0)  # type: ignore[arg-type]
                elif isinstance(m, DecBasicBlock):
                    nn.init.constant_(m.bn2.weight, 0)  # type: ignore[arg-type]

    def _make_layer(self, block: Type[Union[DecBasicBlock, DecBottleneck]], planes: int, blocks: int,
