...
__all__ = ['VisA', 'visa_classes']

def visa_classes():
    return ["candle", "capsules", "cashew", "chewinggum", "fryum", "macaroni1",
            "macaroni2", "pcb1", "pcb2", "pcb3", "pcb4", "pipe_fryum"]


class VisA(Dataset):
    def __init__(self, data_path, learning_mode='centralized', phase='train', 
                 data_transform=None, num_task=12):

        self.data_path = data_path
        self.learning_mode = learning_mode
        self.phase = phase
        self.class_name = visa_classes()
        self.img_transform = data_transform[0]
        self.mask_transform = data_transform[1] 
        assert set(self.class_name) <= set(visa_classes())
