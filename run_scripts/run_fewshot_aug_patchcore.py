...
gpu_id = 2
json_file = '/ssd2/m3lab/usrs/zsh/open-ad/run_scripts/fewshot_patchcore.json'
            ...
            if train_id < int(data[str(fewshot_n)][dataset]):
                ...
                for aug in aug_type:
                    if aug == 'normal':
                        ...
                        script = 'python3 main.py -p c2d -f -fda --fewshot-exm {} -m patchcore -n resnet18 -d {} -tid {} -vid {} -sp {} -g {} -fnd 3 -fat {}'.format(fewshot_n, dataset, train_id, train_id, sample_ratio, gpu_id, aug)
                    # print(script)
                    os.system(script)
                f = open(json_file)
                data = json.load(f)
