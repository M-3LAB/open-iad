    ...
    for fewshot_n, ratio in zip(fewshots, sample_ratios):
        ...
        for train_id in train_ids:
                ...
                if(train_id<int(data[str(fewshot_n)][dataset])):
                    ...
                    script = 'python3 centralized_training.py --fewshot --fewshot-exm {} --model patchcore  -n resnet18 --coreset-sampling-ratio {} --dataset {} --train-task-id {} --valid-task-id {} -g {}'.format(
                                fewshot_n, ratio, dataset, train_id, train_id, gpu_id)
                    # script = 'python3 centralized_training.py --fewshot -fda --fewshot-exm {} --fewshot-num-dg 4 --model patchcore  -n resnet18 --coreset-sampling-ratio {} --dataset {} --train-task-id {} --valid-task-id {} -g {}'.format(
                    #             fewshot_n, ratio, dataset, train_id, train_id, gpu_id)
                    # print(script)
                    os.system(script)
                    f = open('/ssd3/ljq/AD/open-ad/results/fewshot_patchcore.json',) 
