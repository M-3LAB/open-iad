...
gpu_id = 4
json_file = '/ssd2/m3lab/usrs/zsh/open-ad/run_scripts/fewshot_cfa.json'
    ...
    for fewshot_n, ratio in zip(fewshots, sample_ratios):
        ...
        for train_id in train_ids:
            ...
            if train_id < int(data[str(fewshot_n)][dataset]):
                ...
                for aug in aug_type:
                    if aug == 'normal':
                        script = 'python3 main.py -p c2d -f --fewshot-exm {} -m cfa -n net_cfa -d {} -tid {} -vid {} -g {}'.format(fewshot_n, dataset, train_id, train_id, gpu_id)
                    else:
                        script = 'python3 main.py -p c2d -f -fda --fewshot-exm {} -m cfa -n net_cfa -d {} -tid {} -vid {} -g {} -fnd 3 -fat {}'.format(fewshot_n, dataset, train_id, train_id, gpu_id, aug)
                    # print(script)
                    os.system(script)
                f = open(json_file)
                data = json.load(f)
