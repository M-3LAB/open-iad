    ...
    for fewshot_n, ratio in zip(fewshots, sample_ratios):
        ...
        for train_id in train_ids:
            ...
            if train_id < int(data[str(fewshot_n)][dataset]):
                ...
                for aug in aug_type:
                    if aug == 'normal':
                        script = 'python3 main.py -p c2d -f --fewshot-exm {} -m cutpaste -n vit_b_16 -d {} -tid {} -vid {} -g {}'.format(fewshot_n, dataset, train_id, train_id, gpu_id)
                    else:
                        script = 'python3 main.py -p c2d -f -fda --fewshot-exm {} -m cutpaste -n vit_b_16 -d {} -tid {} -vid {} -g {} -fnd 3 -fat {}'.format(fewshot_n, dataset, train_id, train_id, gpu_id, aug)
                    # print(script)
                    os.system(script)
                f = open(json_file)
                data = json.load(f)
