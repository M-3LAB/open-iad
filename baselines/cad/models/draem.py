...
from datasets.transforms.sim_ano_gen import SimulatedAnomalyGeneration
from utils.loss import SSIM, FocalLoss
...
class NetDRAEM(nn.Module):
    def __init__(self, args):
        super(NetDRAEM, self).__init__()
        self.args = args
        self.reconstructive_subnetwork = ReconstructiveSubNetwork(in_channels=3, out_channels=3)
        self.discriminative_subnetwork = DiscriminativeSubNetwork(in_channels=6, out_channels=2)

    def forward(self, aug_img):
        rec_img = self.reconstructive_subnetwork(aug_img)
