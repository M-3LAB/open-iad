...
class PatchCore(torch.nn.Module):
    ...
    #                     image = image["image"]
    #                 input_image = image.to(torch.float).to(self.device)
    #                 features.append(_embed_with_grad(input_image))
    #             return features
    #         return _embed_with_grad(data)
    #
    #     def _embed_with_grad(images):
    #         # _ = self.forward_modules["feature_aggregator"].eval()
    #         # with torch.no_grad():
    #         features = self.forward_modules["feature_aggregator"](images)
    #         # pdb.set_trace()
    ...
    #         return embed_with_grad(input_image)
    #
    #     features = []
    #     optimizer = torch.optim.SGD(self.forward_modules.parameters(), lr=0.001,
    #                                 momentum=0.99)
    ...
    #         self.patch_weight = self._compute_patch_weight(features)
    #         loss = torch.norm(self.patch_weight - 1, p=1)
    #
    ...
    def _fill_memory_bank(self, input_data):
        ...
        if self.softcore_flag:
            with torch.no_grad():
                ...
                # threshold = torch.quantile(patch_weight, 1-self.threshold, dim=1)
                # sampling_weight = torch.where(patch_weight > threshold.unsqueeze(1), 0, 1)
                # self.featuresampler.sampling_weight = sampling_weight.reshape(-1)
                # self.patch_weight = patch_weight.reshape(-1)
                ...
                self.coreset_weight = np.where(kmeans.labels_ == background, 0.1, 1.0)
    ...
    def _compute_patch_weight(self, features: np.ndarray):
        ...
        patch_features = \
            reduced_features.reshape(-1, self.feature_shape[0]*self.feature_shape[1], reduced_features.shape[-1]).permute(1, 0, 2)
        # pdb.set_trace()
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            ...
            if self.softcore_flag:
                if self.weight_method in ["lof", 'nearest', "gaussian"] and self.soft_weight_flag:
                    new_tensor = []
